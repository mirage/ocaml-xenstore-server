...
module Main (C: CONSOLE) = struct
  ...
  let rec logging_thread c logger =
...
let introduce_dom0 () =
  ...
  let rec loop = function
    | "--event" :: e :: _ -> Some (int_of_string e)
    | [] -> None
    | _ :: rest -> loop rest in
...
let start c =
  ...
  let (_: 'a) = logging_thread c Logging.logger in
  let (_: 'a) = logging_thread c Logging.access_logger in
